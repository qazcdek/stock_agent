        ...
        if self.alpha_vantage_key and self.alpha_vantage_key != "your_alpha_vantage_api_key_here":
            try:
                ...
                if bars:
                    ...
                    logger.info("Successfully fetched bars from Alpha Vantage (Tier 1)", ticker=ticker, count=len(bars))
                    return bars
            except Exception as e:
                logger.warning("Alpha Vantage API collection failed, falling back to FinanceDataReader", ticker=ticker, error=str(e))

        # ==========================================================
        # TIER 2: FinanceDataReader (FDR)
        # ==========================================================
        try:
            bars = await self._fetch_from_finance_datareader(ticker, start_date, end_date)
            if bars:
                await self.repository.save_bars(bars)
                logger.info("Successfully fetched bars from FinanceDataReader (Tier 2)", ticker=ticker, count=len(bars))
                return bars
        except Exception as e:
            logger.warning("FinanceDataReader collection failed, falling back to simulated generator", ticker=ticker, error=str(e))
