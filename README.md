...
│   │
│   ├── collectors/                 # ② Periodic Data Gathering
...
│       ├── llm/                    # OpenAI and Anthropic client connectors
...
* **Start Data Collection Daemon**:
  ```bash
  poetry run python apps/data_worker.py
  ```

* **Start Live Trading Daemon**:
  ```bash
  poetry run python apps/trader_worker.py --config configs/paper.yaml
  ```

* **Run a Backtest Simulation**:
  ```bash
  poetry run python apps/backtest_cli.py --ticker 005930 --start 2025-01-01 --end 2026-01-01
  ```
